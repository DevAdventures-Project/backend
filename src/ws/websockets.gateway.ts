import { SubscribeMessage, WebSocketGateway, WsResponse, OnGatewayConnection, OnGatewayDisconnect, WebSocketServer } from '@nestjs/websockets';
...
export class WebsocketsGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    handleMessage(client: Socket, data: string): WsResponse<string> {
      ...
      console.log(`Client with id ${client.id} sent message ${data} to rooms ${clientMapRooms}`);
    ...
    handleJoinRoom(client: any, room: string) {
      ...
      const clientMapRooms = this.getClientMapRooms(client);
      if(clientMapRooms.length > 0 && this.maps.includes(room)) return {
        event: "joinedRoom",
    ...
    handlePosition(client: Socket, data: string): WsResponse<string> {
      const clientMapRooms = this.getClientMapRooms(client);
        console.log(`Client with id ${client.id} sent position ${data} to rooms ${clientMapRooms}`);
      clientMapRooms.forEach((room: string) => {
    ...
    private getClientMapRooms(client: any) {
        return Array.from(client.rooms).filter((room: string) => this.maps.includes(room));
    }
}